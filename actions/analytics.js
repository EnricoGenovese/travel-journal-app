...
export async function getMoodAnalytics(period = "30d") {
  ...
  const analitycsData = Object.entries(moodData).map(([date, data]) => ({
    date,
  ...
  const overallStats = {
    ...
    dailyAverage: Number((entries.length / (period === "7d" ? 7 : period === "15d" ? 15 : 30))
    ),
  };
